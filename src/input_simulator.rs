...
use enigo::Keyboard;
use enigo::{Enigo, Settings, agent::{Agent, Token}, Direction, Key};
...
impl InputSimulator {
                ...
                "wayland" => {
                    ...
                    if let Ok(desktop) = std::env::var("XDG_CURRENT_DESKTOP") {
                        match desktop.to_lowercase().as_str() {
                            "gnome" => {
                                println!("   🔧 GNOME detected - may work with accessibility permissions enabled");
                            },
                            "kde" => {
                                println!("   🔧 KDE Plasma detected - input simulation usually works well");
                            },
                            "sway" => {
    ...
    pub fn execute_actions(&mut self, actions: &[ExecutableAction]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn execute_key_press(&mut self, key: Key, auto_release: bool) -> Result<(), Box<dyn std::error::Error>> {
        let press_token = Token::Key(key.clone(), Direction::Press);
    ...
    fn execute_key_release(&mut self, key: Key) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if self.pressed_keys.contains(&key) {
            ...
            println!("Key {:?} not currently pressed, skipping release", key);
        }
    ...
    fn schedule_release_all_after(&mut self, duration_ms: u64) {
        ...
        for key in &self.pressed_keys {
            self.scheduled_releases.push((release_time, key.clone()));
        }
    ...
    pub fn process_scheduled_releases(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.scheduled_releases.retain(|(release_time, key)| {
            if *release_time <= now {
                releases_to_process.push(key.clone());
